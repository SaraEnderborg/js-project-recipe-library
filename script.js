...
const filterOnCuisine = () => {
  ...
  const filteredRecipes =
    selectedCuisine === "all"
      ...
      : recipesArray.filter(
          (recipe) =>
            recipe.cuisines.map((cuisine)=> cuisine.toLowerCase()).includes(selectedCuisine.toLowerCase())
        );
...
const sortOnTime = () => {
  ...
  const sorted = [...recipesArray].sort((a, b) => {
    return order === "asc" ? a.readyInMinutes - b.readyInMinutes : b.readyInMinutes - a.readyInMinutes;
  });
